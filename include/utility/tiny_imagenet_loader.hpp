...
template<typename T>
static std::vector<std::pair<std::vector<T>, std::vector<T>>> load_tiny_imag_train(
    const std::string & dirname, const std::unordered_map<std::string, int> & category_map){

    std::vector<std::pair<std::vector<T>, std::vector<T>>> training_data{};
    ...
    for (const auto& subdir : std::filesystem::directory_iterator(base_directory)) {
        ...
        for (const auto& image_file : std::filesystem::directory_iterator(subdir.path() / "images")){
            ...
            std::vector<T> imag_vec = img_vec_convert<T>(image_file.path().string());
...
template<typename T>
static std::vector<std::pair<std::vector<T>, std::vector<T>>> load_tiny_imag_test(
    const std::string & dirname, const std::unordered_map<std::string, int> & category_map){
